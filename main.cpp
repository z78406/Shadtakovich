...
Eigen::Vector3f texture_fragment_shader(const fragment_shader_payload& payload, const light::p_Light& p_light, const light::a_Light& a_light, 
    ...
    for (int i = 0; i < pl_pos.size(); i++)
    {
        ...
        Eigen::Vector3f view_dir  = - point;   // from surface point to view
...
int main(int argc, const char** argv) {
    ...
    Eigen::Vector3f angle = {30.0, 120.0, 0.0};			                            // set rotation (world -> camera)  
    ...
    std::string shdow_filename = "output_shadow.png";                           // output img file name
	objl::Loader Loader;
    ...
    std::vector<std::vector<float>> shadow_depth_buf_list;   
    ...
    if (use_shadow_mapping) {
        ...
        for (int i = 0; i < point_light_pos.size(); i++) {
            ...
            r_shadow.set_model(Tf->get_model_matrix(light_angle[i], light_scale[i], light_translation[i])); // world to camera space
            r_shadow.set_view(Tf->get_view_matrix(point_light_pos[i], point_light_gaze[i], point_light_up[i]));				  // light to light canonical 
            ...
            // const Eigen::Matrix4f s_model = shadow_transform[0], s_view = shadow_transform[1];   // get transformation matrix 
            // Eigen::Vector3f new_point_light_pos = p_l.p_trans(p_l.position[i], s_view * s_model); // update light pos from world to light space
            // p_l.position[i] = new_point_light_pos;                             
            r_shadow.drawTriangle(TriangleList, p_l, a_l, point_light_pos[i]);
            shadow_buf = r_shadow.frame_buffer();                                               // obtain shadow buf data
            shadow_depth_buf = r_shadow.depth_buffer();                                         // obtain shadow depth buf data
            shadow_buf_list.push_back(shadow_buf);
            shadow_depth_buf_list.push_back(shadow_depth_buf);            
            cv::Mat image(screen_width, screen_height, CV_32FC3, r_shadow.frame_buffer().data());
            image.convertTo(image, CV_8UC3, 1.0f); 
            cv::imshow("image", image);
            cv::imwrite(shdow_filename, image);
            cv::waitKey(0);   
    ...
    if (command_line) { 										                                            // extra command from cmd line
        ...
        // std::cout<<p_l.position[0]<<std::endl<<new_point_light_pos[0]<<std::endl;
        // Eigen::Vector3f p3_new = p_l.p_trans(p_l.position[0], model);
        // Eigen::Vector4f after_model_transform = p_l.position[0] * model;
        // std::cout<<p3_new<<std::endl;
        // return -1;
        // std::cout<<"light direction:"<<new_point_light_pos<<std::endl;
